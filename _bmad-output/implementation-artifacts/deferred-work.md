...
## Halted: Epic 4 — Task-Label Taxonomy Cleanup (evidence gate NO-GO, 2026-07-13)

- **Merge of `general`/`knowledge`/`factual` NOT justified by the data** — Story 4.1's evidence slice (real materialized run, 2716 true-trio rows, held-out 20%) returned **NO-GO**: internal `trio_confusion_rate=0.085` < provisional threshold 0.15, while `trio_escape_rate=0.117` (trio→`other`) dominates. Merging collapses the labels *within* the trio, so it cannot fix escape — which is the larger error. Per-class, `general` is the broken label (recall 0.033) but its failure is escape, not inter-trio confusion, so the proposed merge treats the wrong disease. Epic 4 halted per its own evidence-gate design (user-confirmed 2026-07-13); 4.2/4.3 → wont-do. Evidence: `evaluation/task_type_slice/go_no_go_decision.md`. **Reopen only if:** a follow-up slice shows what the trio escapes *to* and identifies a label change that reduces escape (the actual weakness); or the weak dataset-derived labels are replaced with stronger ones and re-measured. Threshold was provisional (SQ-1), but 0.085 is ~half of it and the escape>confusion split argues against merging regardless of threshold.
